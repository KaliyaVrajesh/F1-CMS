...
const LiveRace = () => {
  ...
  // Load upcoming race circuit on mount
  useEffect(() => {
    getF1NextRace()
      .then(({ data }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      >
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-gray-400 font-mono text-xs">VIRTUAL BROADCAST</span>
          </div>

          <h1 className="text-3xl sm:text-4xl font-f1heading font-black text-white uppercase tracking-wider">
            <span className="text-f1red">Live</span> Race Visualizer
          </h1>
        </div>

        {/* Circuit Dropdown Selector */}
        <div className="flex items-center gap-2">
          <label className="text-xs font-mono text-gray-400 uppercase hidden sm:inline">
            Circuit:
          </label>
          <select
            value={selectedCircuitKey}
            onChange={(e) => handleCircuitChange(e.target.value)}
            className="bg-dark-900 border border-white/15 text-white text-xs font-mono px-3 py-2 rounded-xl focus:outline-none focus:border-amber-500 cursor-pointer shadow-lg"
          >
      ...
      <div className="p-3 rounded-2xl bg-[#090b10] border border-white/10 flex flex-wrap items-center justify-between gap-3 shadow-xl">
        ...
        {/* Right: Driver Focus Hint */}
        <div className="text-[11px] font-mono text-gray-400">
          {selectedDriver ? (
            <span className="text-amber-400 font-bold">
              🎯 Focusing: {selectedDriver.name}
            </span>
          ) : (
            <span>💡 Click any driver dot to view telemetry</span>
          )}
        </div>
      </div>
      ...
      <LiveRaceHUD
        ...
        totalLaps={circuitDetails.totalLaps}
